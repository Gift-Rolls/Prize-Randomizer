...
def main(data_path, seed):
    ...
    print(f"Today we will be giving away {len(winners)} prizes", end="")
    input()
    ...
    print("Here're the luckiest Rolls users:", end="")
    print()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Rolls Trading Competition prize selection')
    parser.add_argument('--data', type=str, required=True, help='Path to the .csv file')
    parser.add_argument('--seed', type=int, required=True, help='Random seed')  # get from 5 dice rolls in @rolls_tv on live stream
